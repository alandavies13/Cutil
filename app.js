...
function render(list, qRaw){
  ...
  for (const r of list){
    ...
    left.innerHTML = `
      ...
      <div class="k2">${r.area || ""}${r.funcao ? " • " + r.funcao : ""}</div>
    `;
    ...
    // Informações visíveis diretamente no Card
    const pPainel = document.createElement("div");
    pPainel.className = "pill";
    pPainel.textContent = `Painel: ${r.painel || "—"}`;

    const pCubic = document.createElement("div");
    pCubic.className = "pill";
    pCubic.textContent = `Cubículo/Gaveta: ${r.cubic || "—"}`;

    const pVemDe = document.createElement("div");
    pVemDe.className = "pill";
    pVemDe.textContent = `Vem de: ${r.vem_de || "—"}`;

    const pAlimenta = document.createElement("div");
    pAlimenta.className = "pill";
    pAlimenta.textContent = `Alimenta: ${r.vai_para || "—"}`;

    row.appendChild(pPainel);
    row.appendChild(pCubic);
    row.appendChild(pVemDe);
    row.appendChild(pAlimenta);
