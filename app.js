...
function normalizeRecord(raw){
  return {
    ...
    vai_para: toEmptyIfNull(raw["Vai para"]), // mantém a chave do JSON
...
function findHitLabel(r, qRaw){
  ...
  const fields = [
    { key: "vai_para", label: "Alimenta" },     // <-- alterado aqui
...
function render(list, qRaw){
  ...
  for (const r of list){
    ...
    p2.className = "pill";
    p2.textContent = `Alimenta: ${r.vai_para || "—"}`; // <-- alterado aqui
...
function openDetails(r){
  ...
  el("dlg_body").innerHTML = [
    ...
    kv("Vem de", r.vem_de),
    kv("Alimenta", r.vai_para), // <-- alterado aqui
    kv("Grau", r.grau),
