...
function findHitLabel(r, qRaw){
  ...
  // Se houver busca por outro campo específico, exibe a correspondência
  const fields = [
    { key: "vai_para", label: "" }, // sem prefixo para o destino principal
...
function buildCardTitle(r, qRaw){
  ...
  // 1. Se houver busca e bater com algum campo, exibe a correspondência
  const hit = findHitLabel(r, qRaw);
  if (q && hit) return hit;

  // 2. Título principal: Apenas o valor direto de "vai_para" (Alimenta), sem prefixo
  if (r.vai_para) {
    return r.vai_para;
  }

  // 3. Fallback se não houver o campo "vai_para"
...
function render(list, qRaw){
  ...
  for (const r of list){
    ...
    // Subtítulo contendo apenas os dados diretos de Painel e Cubículo/Gaveta
    ...
    left.innerHTML = `
      <div class="k1" style="display: flex; align-items: center; gap: 8px;">
        ...
        <span class="badge">${r.tensao_v ? `${r.tensao_v} V` : "—"}</span>
      </div>
    ...
    // Apenas a pill do "Vem de" na parte inferior do card
...
function openDetails(r){
  // Título do modal contendo apenas os dados de Painel e Cubículo/Gaveta
