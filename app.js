...
function render(list, qRaw){
  ...
  for (const r of list){
    ...
    // Dados do subtítulo: apenas valores de Painel e Cubículo/Gaveta
    const subInfo = [r.painel, r.cubic].filter(Boolean).join(" • ");

    const left = document.createElement("div");
    left.style.width = "100%";
    left.innerHTML = `
      <div class="k1" style="display: flex; align-items: center; justify-content: space-between; gap: 8px;">
        <span>${buildCardTitle(r, qRaw)}</span>
        <span class="badge" style="flex-shrink: 0;">${r.tensao_v ? `${r.tensao_v} V` : "—"}</span>
      </div>
...
function openDetails(r){
  ...
  // Subtítulo do modal com Área e Função
