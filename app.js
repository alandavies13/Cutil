...
function render(list, qRaw){
  ...
  for (const r of list){
    ...
    // Formatação da Tensão no subtítulo
    const tensaoStr = r.tensao_v ? `${r.tensao_v} V` : "";

    // Subtítulo: Painel • Cubículo/Gaveta • Tensão
    const subInfo = [r.painel, r.cubic, tensaoStr].filter(Boolean).join(" • ");
...
function openDetails(r){
  ...
  // Subtítulo do modal: Substituído por "Vem de: [valor]" + Função (se houver)
